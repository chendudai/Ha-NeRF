...
class WikiScenesDataset(Dataset):
    def __init__(self, root_dir, split='train', img_downscale=1, val_num=1, use_cache=False):

        """
        img_width: what the modified width of the images is.
    ...
    def read_meta(self):
        ...
        self.files = self.files[:1000]  # start with smaller set
        ...
        if self.use_cache:
            ...
            imdata = read_images_text(os.path.join(self.root_dir, 'dense/sparse/images.txt'))
        ...
        if self.use_cache:
            ...
            self.Ks = {}  # {id: K}
            camdata = read_cameras_text(os.path.join(self.root_dir, 'dense/sparse/cameras.txt'))
            ...
            for i,id_ in enumerate(self.img_ids):
                ...
                if cam is not None:
                    ...
                    K[2, 2] = 1
                    # img_w_ = img_w  # self.img_width
                    # img_h_ = img_h  # int(img_h * (img_w_ / img_w))
                    # # simple radial model: f, cx, cy, k (pinhole mode: fx, fy, cx, cy)
                    # # ignores distortion
        ...
        if self.use_cache:
            ...
            pts3d = read_points3D_text(os.path.join(self.root_dir, 'dense/sparse/points3D.txt'))
        ...
        print('Step 5. split the img_ids (the number of images is verfied to match that in the paper)')
...
class PhototourismDataset(Dataset):
    ...
    def read_meta(self):
        ...
        if self.use_cache:
            ...
            if os.path.isfile(os.path.join(self.root_dir, 'dense/sparse/images.bin')):
        ...
        if self.use_cache:
            ...
            self.Ks = {} # {id: K}
            if os.path.isfile(os.path.join(self.root_dir, 'dense/sparse/cameras.bin')):
        ...
        if self.use_cache:
            ...
            if os.path.isfile(os.path.join(self.root_dir, 'dense/sparse/points3D.bin')):
